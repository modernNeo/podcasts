    ...
    def run(self, information):
        ...
        if os.path.exists(full_path):
            ...
            if podcast_being_processed:
                ...
                youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {current_file_name=}")
                timestamp, cbc_vancouver_news_video = get_timestamp(
                    information, current_file_name,podcast_being_processed
                )
                if timestamp:
                    youtube_dlp_logger.info(
                    ...
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {new_file_name=}")

                    index_of_last_period = new_file_name.rfind('.')
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {index_of_last_period=}")
                    ...
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] cleaned {new_file_name=}")
                    old_file_path = f"{full_path[:slash_indices[number_of_slashes - 1] + 1]}{current_file_name}"
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {old_file_path=}")
                    new_file_path = f"{full_path[:slash_indices[number_of_slashes - 1] + 1]}{new_file_name}"

                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {new_file_path=}")

                    os.rename(old_file_path, new_file_path)
                    release_stamp = int(timestamp.strftime('%Y-%m-%d-%H-%M').replace("-", ""))
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {release_stamp=}")
                    grouping_release_stamp = int(timestamp.strftime('%Y-%m-%d').replace("-", ""))
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {grouping_release_stamp=}")

                    original_title = information.get('title', None)
                    description = information.get('description', None)
                    original_url = information.get('original_url', None)
                    filesize = information.get('filesize', None)
                    filesize_approx = information.get('filesize_approx', None)
                    duration = information.get('duration', None)

                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {original_title=}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {description=}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] podcast={podcast_being_processed}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {original_url=}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {thumbnail=}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {filesize=}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {filesize_approx=}")
                    youtube_dlp_logger.info(f"[youtube_video_post_processor.py run()] {duration=}")
                    if filesize:
                        file_size = filesize
                        ...
                        youtube_dlp_logger.warn(
                            f"could not find the file size for {original_title} with a url of {original_url}"
                        )
                    ...
                    youtube_podcast_video.original_title = original_title
                    youtube_podcast_video.description = description
                    ...
                    youtube_podcast_video.url = original_url
                    ...
                    youtube_podcast_video.duration = duration
